...
sap.ui.define([
	...
	return Controller.extend("MOBI.controller.Configuration", {
		...
		onChangeRole: function(Event) {
			var oModelRoleLocation = new sap.ui.model.json.JSONModel();
			var item = this.getView().byId("cboxIdRoleAtt").getSelectedItem();
			var data = item.getBindingContext("oModelRoleAtt").getProperty();
			//var data = this.getView().getModel("oModelRoleAtt").getProperty(path);

			if (this.getView().byId("cboxIdRolePosition").setSelectedKey() !== "") {
				this.getView().byId("cboxIdRolePosition").setSelectedKey("");
				this.getView().byId("cboxIdAttMenu").setSelectedKey("");
		...
		getRolePositionValue: function(data) {
			...
			$.ajax({
				...
				success: function(osuccess) {
					...
					var AttrMenu = that.getView().byId("cboxIdAttMenu");
					...
					//that.getView().byId("cboxIdAttMenu").setSelectedKey("");
					/*oModelRoleLocation.setData([]);
		...
		onPressSubmitSubMenu: function() {
			...
			var that = this;
			var menuId = sap.ui.getCore().byId("cAddBoxMenuSubMenu").getSelectedKey();
			var subMenu = sap.ui.getCore().byId("menuSubMenu").getValue().trim();
			var ordernosubMenu = sap.ui.getCore().byId("ordernoSubMenu").getValue().trim();
			var keySub = sap.ui.getCore().byId("keySubMenu").getValue().trim();
			var iconSub = sap.ui.getCore().byId("iconSubMenu").getValue().trim();
			if (subMenu === "" || ordernosubMenu === "" || keySub === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (subMenu === "") {
				sap.m.MessageBox.show(" Kindly Select Sub Menu", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (ordernosubMenu === "") {
				sap.m.MessageBox.show(" Kindly Enter Order No SubMenu", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (keySub === "") {
				sap.m.MessageBox.show(" Kindly Enter KeySub", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
		...
		onPressSubmitLevel: function() {
			...
			if (level === "") {
				sap.m.MessageBox.show(" Kindly Enter Level", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			busyDialog2 = new sap.m.BusyDialog("", {
				text: "processing please wait...",
			...
			busyDialog2.open();
			$.ajax({
				url: customerTypeAPIPath + "Level.xsjs?cmd=validateLevel",
				type: "POST",
				data: {
					"level": level
				},
		...
		onPressSubmitRole: function() {
			var that = this;
			var roleName = sap.ui.getCore().byId("roleAddRole").getValue().trim();

			if (roleName === "") {
				sap.m.MessageBox.show("Kindly Enter Role", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			...
			busyDialog2.open();
			var surlRole = customerTypeAPIPath + "Role.xsjs?cmd=validateRole";
			$.ajax({
				url: surlRole,
				type: "POST",
				data: {
				...
				error: function(oerr) {
					...
					jQuery.sap.require("sap.ui.commons.MessageBox");
					// open a fully configured message box
					sap.ui.commons.MessageBox.show("Unable to Insert Role. Kindly contact Admin", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				}
		...
		onPressSubmitLevelAttribute: function() {
			...
			var data;
			var record = {};
			record.levelId = this.getView().byId("cboxLev").getSelectedKey();
			record.MENU_ID = this.getView().byId("cboxIdLevAttMenu").getSelectedKey();
		...
		onPressSubmitRolePositionAttribute: function() {
			var data;
			var dataArray = this.getView().byId("tableRoleLocsubmenu").getModel("oModelRoleLocAttTable").getData().results;
			if (dataArray.length > 0) {
				for (var i = 0; i < dataArray.length; i++) {
					var ROLE_LOC_ID = this.getView().byId("cboxIdRolePosition").getSelectedKey();
					//data.SUBID = dataArray[i].SUBID;
					var MENU_ID = this.getView().byId("cboxIdAttMenu").getSelectedKey();
					if (dataArray[i].INSERTCHECK === true) {
						data = {};
						data.ID = dataArray[i].INSERT_ID;
						data.SUBID = dataArray[i].SUBID;
		...
		RefreshDataRoleAttri: function() {
			...
			this.getView().byId("cboxIdRolePosition").setSelectedKey("");
			this.getView().byId("cboxIdAttMenu").setSelectedKey("");
			this.getView().byId("cboxIdRoleAtt").setSelectedKey("");
			this.onDataReceivedRoleLocationAttribute();
		},
		RefreshData: function() {
			...
			this.getView().byId("cboxIdLevAttMenu").setSelectedKey("");
			//this.getLevelsubMenuAttri();
		},

		onPressSubmitAttribute: function() {
			var that = this;
			var attribute = sap.ui.getCore().byId("idAttribute").getValue().trim();
			var display = sap.ui.getCore().byId("idDisplay").getSelectedKey();
			if (attribute === "") {
				sap.m.MessageBox.show(" Kindly Enter Attribute", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			busyDialog2 = new sap.m.BusyDialog("", {
				text: "processing please wait...",
		...
		OnPressCustomerTypeFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				filters = [new sap.ui.model.Filter("CUST_TYPE", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CUST_TYPE_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SOFT_DEL_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CREATE_DATE", sap.ui.model.FilterOperator.Contains, sQuery)
				];
				var filter = new sap.ui.model.Filter(filters, false);
			}
			this.getView().byId("IdTableCustomerType").getBinding("items").filter(filter);
		...
		OnPressAttributeFilterNew: function(Event) {
			//	var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				filters = [new sap.ui.model.Filter("ATTRIBUTE_NAME", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SOFT_DEL_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CREATE_DATE", sap.ui.model.FilterOperator.Contains, sQuery)
				];
				var filter = new sap.ui.model.Filter(filters, false);
			}
			this.getView().byId("IdTableAttribute").getBinding("items").filter(filter);
		...
		OnPressLevelFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
			...
			this.getView().byId("IdTableLevel").getBinding("items").filter(filter);
		...
		OnPressRoleFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				filters = [new sap.ui.model.Filter("ROLENAME", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SOFT_DEL_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CREATEDATE", sap.ui.model.FilterOperator.Contains, sQuery)
				];
				var filter = new sap.ui.model.Filter(filters, false);
			}
			this.getView().byId("IdTableRole").getBinding("items").filter(filter);
		...
		OnPressPositionFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
			...
			this.getView().byId("IdTablePosition").getBinding("items").filter(filter);
		...
		onPressSubmitCustomerType: function() {
			var that = this;
			var custtype = sap.ui.getCore().byId("CustTypeCustomerType").getValue().trim();
			var custdesc = sap.ui.getCore().byId("CustDescCustomerType").getValue().trim();
			
				if (custtype === "" || custdesc === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (custtype === "") {
		...
		onChangeLevelAttr: function(Event) {
			var menu = this.getView().byId("cboxIdLevAttMenu");
			var that = this;
			if (menu.getValue() !== "") {
				menu.setSelectedKey("");
				var oModel = new sap.ui.model.json.JSONModel();
		...
		OnPressMenuFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				filters = [new sap.ui.model.Filter("MENU_NAME", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("URL", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SOFT_DEL_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CREATE_DATE", sap.ui.model.FilterOperator.Contains, sQuery)
				];
				var filter = new sap.ui.model.Filter(filters, false);
			}
			this.getView().byId("IdTableMenu").getBinding("items").filter(filter);
		...
		OnPressSubMenuFilterNew: function(Event) {
			var aFilters = [];
			var filters;
			var sQuery = Event.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				filters = [new sap.ui.model.Filter("MENUNAME", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SUB_MENU_NAME", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("SOFT_DEL_DESC", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("KEY", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("ICON", sap.ui.model.FilterOperator.Contains, sQuery),
					new sap.ui.model.Filter("CREATE_DATE", sap.ui.model.FilterOperator.Contains, sQuery)
				];
				var filter = new sap.ui.model.Filter(filters, false);
			}
			this.getView().byId("IdTableSubMenu").getBinding("items").filter(filter);
		...
		onPressSubmit_LevAttrMap: function() {
			...
			var levelLevAttrMap = this.getView().byId("cboxLev").getSelectedKey();
			var menuLevAttrMap = this.getView().byId("cbox_Menu").getSelectedKey();
			var surl1 = "/xsService/MobiAPI/config/MapLevelAttr.xsjs?cmd=addMapLevAttr";
			busyDialog2 = new sap.m.BusyDialog("", {
				text: "processing please wait...",
				title: ""
			...
			$.ajax({
				...
				type: "POST",
				data: {
					"levelId": levelLevAttrMap,
					"SubMenuId": menuLevAttrMap
				},
		...
		onPressSubmitMenu: function() {
			var that = this;
			var menuMenu = sap.ui.getCore().byId("menuMenu").getValue().trim();
			var ordernoMenu = sap.ui.getCore().byId("ordernoMenu").getValue().trim();
			var urlMenu = sap.ui.getCore().byId("urlMenu").getValue().trim();

			var surl1 = customerTypeAPIPath + "Menu.xsjs?cmd=validateMenu";
			if (menuMenu === '' && ordernoMenu === '' && urlMenu === '') {
				sap.ui.commons.MessageBox.show("Please fill all details", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (menuMenu === '') {
				sap.ui.commons.MessageBox.show("Please fill Menu name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (ordernoMenu === '') {
				sap.ui.commons.MessageBox.show("Please fill order No.", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (urlMenu === '') {
				sap.ui.commons.MessageBox.show("Please fill Url", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			busyDialog2 = new sap.m.BusyDialog("", {
				text: "processing please wait...",
		...
		onPressSubmitPosition: function() {
			var that = this;
			var positionPositionName = sap.ui.getCore().byId("positionPosition").getValue().trim();
			if (positionPositionName === "") {
				sap.m.MessageBox.show(" Kindly Enter Position", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}

			var surl1 = customerTypeAPIPath + "Position.xsjs?cmd=validatePosition";
		...
		onDialogSaveButtonRoleEdit: function() {

			var that = this;
			var editRoleID = sap.ui.getCore().byId("roleIdRoleEdit").getValue().trim();
			var editRoleNAME = sap.ui.getCore().byId("roleNameRoleEdit").getValue().trim();
			var editRoleStatus = sap.ui.getCore().byId("roleCboxStatus").getSelectedKey();
			if (editRoleNAME === "") {
				sap.m.MessageBox.show(" Kindly Enter Role", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editRoleStatus === "") {
				editRoleStatus = that.defaultStatus(sap.ui.getCore().byId("roleCboxStatus"));
			}
			busyDialog2 = new sap.m.BusyDialog("", {
				text: "processing please wait...",
				title: ""
		...
		onDialogSaveButtonPositionEdit: function() {
			...
			var editPositionID = sap.ui.getCore().byId("positionIdPositionEdit").getValue().trim();
			var editPositionName = sap.ui.getCore().byId("positionNamePositionEdit").getValue().trim();
			var editPositionStatus = sap.ui.getCore().byId("positionCboxStatus").getSelectedKey();
			if (editPositionName === "" || editPositionStatus === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editPositionName === "") {
				sap.m.MessageBox.show(" Kindly Select Position Name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}

			if (editPositionStatus === "") {
				editPositionStatus = that.defaultStatus(sap.ui.getCore().byId("positionCboxStatus"));
			}
		...
		onDialogSaveButtonLevelEdit: function() {
			...
			var editLevelID = sap.ui.getCore().byId("levelIdLevelEdit").getValue().trim();
			var editLevelNAME = sap.ui.getCore().byId("levelNameLevelEdit").getValue().trim();
			var editLevelStatus = sap.ui.getCore().byId("levelCboxStatus").getSelectedKey();

			if (editLevelNAME === "") {
				sap.m.MessageBox.show(" Kindly Select Level Name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}

			if (editLevelStatus === "") {
				editLevelStatus = that.defaultStatus(sap.ui.getCore().byId("levelCboxStatus"));
				//return;
			}
			...
			$.ajax({
				...
				data: {
					...
					"levelNameEditStatus": editLevelStatus
				},
		...
		onDialogSaveButtonEditMenu: function() {
			...
			var editMenuNAME = sap.ui.getCore().byId("menuNameMenuEdit").getValue().trim();
			var editOrderNo = sap.ui.getCore().byId("menuOrderNoMenuEdit").getValue().trim();
			var editURL = sap.ui.getCore().byId("menuURLMenuEdit").getValue().trim();
			var editStatus = sap.ui.getCore().byId("cboxStatusEditMenu").getSelectedKey();
			if (editMenuNAME === "" || editOrderNo === "" || editURL === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editMenuNAME === "") {
				sap.m.MessageBox.show(" Kindly Enter Menu Name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editOrderNo === "") {
				sap.m.MessageBox.show(" Kindly Enter Order No.", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editURL === "") {
				sap.m.MessageBox.show(" Kindly Enter URL", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editStatus === "") {
				editStatus = that.defaultStatus(sap.ui.getCore().byId("cboxStatusEditMenu"));
			}
		...
		onDialogSaveButtonEditSubMenu: function() {
			...
			var editSubMenuNAME = sap.ui.getCore().byId("subMenuNameSubMenuEdit").getValue().trim();
			var editSubOrderNo = sap.ui.getCore().byId("subMenuOrderNoSubMenuEdit").getValue().trim();
			var editSubKey = sap.ui.getCore().byId("subMenuKeySubMenuEdit").getValue().trim();
			var editSubIcon = sap.ui.getCore().byId("subMenuIconSubMenuEdit").getValue().trim();
			var editSubStatus = sap.ui.getCore().byId("cboxStatusEditSubMenu").getSelectedKey();
			if (editSubMenuNAME === "" || editSubOrderNo === "" || editSubKey === "" || editSubIcon === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editSubMenuNAME === "") {
				sap.m.MessageBox.show(" Kindly Enter SubMenu Name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editSubOrderNo === "") {
				sap.m.MessageBox.show(" Kindly Enter SubOrder No.", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editSubKey === "") {
				sap.m.MessageBox.show(" Kindly Enter SubKey", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editSubIcon === "") {
				sap.m.MessageBox.show(" Kindly Enter SubIcon", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editSubStatus === "") {
				editSubStatus = that.defaultStatus(sap.ui.getCore().byId("cboxStatusEditSubMenu"));
			}
		...
		onEditPressAttribute: function(oEvt) {
			...
			propEquipment = context.getProperty();
			var oModelEditAttribute = new sap.ui.model.json.JSONModel();
			oModelEditAttribute.setData(propEquipment);
			sap.ui.getCore().setModel(oModelEditAttribute, "oModelEditAttribute");
		...
		onDialogSaveButtonEditAttribute: function() {
			...
			var editAttributeID = sap.ui.getCore().byId("attributeIdAttributeEdit").getValue().trim();
			var editAttibuteNAME = sap.ui.getCore().byId("attrubuteNameAttributeEdit").getValue().trim();
			var editDisplay = sap.ui.getCore().byId("attrubuteDisplayAttributeEdit").getValue().trim();
			var editStatusattr = sap.ui.getCore().byId("cboxStatusEditAttribute").getSelectedKey();

			if (editAttibuteNAME === "") {
				sap.m.MessageBox.show(" Kindly Enter Atrribute Name", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}

			if (editStatusattr === "") {
				editStatusattr = that.defaultStatus(sap.ui.getCore().byId("cboxStatusEditAttribute"));
			}
		...
		onEditPressLevelHierarchy: function(oEvt) {
			...
			propEquipment = context.getProperty();
			var oModelEditLevelHierarchy = new sap.ui.model.json.JSONModel();
			oModelEditLevelHierarchy.setData(propEquipment);
			sap.ui.getCore().setModel(oModelEditLevelHierarchy, "oModelEditLevelHierarchy");
		...
		onDialogSaveButtonCustomerTypeEdit: function() {
			...
			var editCustType = sap.ui.getCore().byId("CustTypeCustomerTypeEdit").getValue().trim();
			var editCustTypeDesc = sap.ui.getCore().byId("CustTypeDescCustomerTypeEdit").getValue().trim();
			var editStatus = sap.ui.getCore().byId("cboxStatusEditCustType").getSelectedKey();
				if (editCustType === "" || editCustTypeDesc === "") {
				sap.m.MessageBox.show(" Kindly Select All the Fields", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editCustType === "") {
				sap.m.MessageBox.show(" Kindly Enter CustType", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editCustTypeDesc === "") {
				sap.m.MessageBox.show(" Kindly Enter CustTypeDesc", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
				return;
			}
			if (editStatus === "") {
				editStatus = that.defaultStatus(sap.ui.getCore().byId("roleCboxStatus"));
			}
